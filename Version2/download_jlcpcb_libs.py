#!/usr/bin/env python3
"""
Download JLCPCB/LCSC symbols and footprints for all parts in jlc_parts_enriched.json
...
def load_parts():
    ...
    for part in data["parts"]:
        ...
        if lcsc:
            ...
            mpn = part.get("selection", {}).get("mpn") or part.get("query", "")
            if lcsc_clean not in lcsc_parts:
                lcsc_parts[lcsc_clean] = {
                    "designators": [],
                    "mpn": mpn
                }
            lcsc_parts[lcsc_clean]["designators"].append(designator)
...
def register_global_libraries():
    ...
    sym_entry = f'  (lib (name "{SYMBOL_LIB}")(type "KiCad")(uri "{sym_path}")(options "")(descr "JLCPCB/LCSC parts library"))'

    # Footprint library entry
    fp_entry = f'  (lib (name "{FOOTPRINT_LIB}")(type "KiCad")(uri "{fp_path}")(options "")(descr "JLCPCB/LCSC footprints library"))'
...
def main():
    ...
    for lcsc, info in lcsc_parts.items():
        print(f"  {lcsc}: {', '.join(info['designators'])} ({info['mpn']})")
